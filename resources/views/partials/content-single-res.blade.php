        ...
        @if (!empty($r->link))
            @if ($r->mature)
                <a class="no_link_effect"
                    ...
                    <div
                        class="inline-block mt-4 p-4 mx-auto hover:bg-{{ $r->type_label }} text-center uppercase font-bold text-lg transition-colors duration-150">
            ...
            @else
                <a class="no_link_effect" href="{{ $r->link }}" target="_blank">
                    <div
                        class="inline-block mt-4 p-4 mx-auto rounded-lg hover:bg-{{ $r->type_label }} text-center uppercase font-bold text-lg transition-colors duration-150">
                        <span>{{ pll__('Accéder à la ressource') }}&nbsp;🔗</span>
                    </div>
                </a>
            @endif
        ...
        <div class="entry-content relative">
            ...
            @if (!empty($r->get_yt_id()))
                <div class="relative overflow-hidden h-0 my-6" style="padding-bottom:56.25%">
                    <iframe class="w-full h-full absolute left-0 top-0" id="ytplayer" type="text/html" width="640"
                        height="360"
